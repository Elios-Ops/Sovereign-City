...
- **Infinite Agentic Loop** (standalone site) — source selected (`AI_APP/infinite-agentic-loop-site/`), destination `apps/infinite-agentic-loop/` created; import in progress via a resumed background agent as of this state file.
...
- `apps/muskrats/` entry point and "ENTER THE SEWER" destination both run locally and are confirmed end-to-end; most of the rest of the site is still pending import.
...
Finish importing the remaining ~46 Muskrats.io pages/assets and the Infinite Agentic Loop site from the Drive folder IDs recorded in `docs/recovery/CANONICAL_SOURCE_MAP.md` (the mechanism and safety rules — secrets exclusion, oversized-binary logging — are established and repeatable). Then run Infinite Agentic Loop locally and extend the connection registry to cover it and the remainder of Muskrats.io.
